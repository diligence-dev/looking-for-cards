...
|---|---|---|---|
| `GET` | `/api/entries` | `200 {"entries":[…], "total":N}` (all entries, server-sorted; when `?user=<name>` is supplied, only entries where the seeker shares at least one occasion with the viewer are returned — entries with no occasions are always visible, viewers with no occasions see all) | — |
...
```sh
CGO_ENABLED=1 go test ./server/tests/   # 59 tests
```
