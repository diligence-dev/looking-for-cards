...
- **Screen B** (gallery): sticky top bar with `@<user>` + change link, Refresh, card-name filter, color filter, card-type filter, seeker-name multiselect, giver-name multiselect, "Hide giver found" checkbox. Grid of tiles + "Show more" (PAGE_SIZE = 50).
