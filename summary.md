...
| `set_codes` | TEXT | Comma-separated sorted list of every set code the card name was printed in (e.g. `2ED,FRA,LEA`), backfilled client-side from Scryfall; `""` = not yet backfilled |
...
| `note` | TEXT | Optional printing note (max 50 characters), e.g. "English, M15 frame"; `""` = none. Trimmed, truncated to 50 characters (runes) server-side; counted/truncated to 50 code points client-side (`maxlength=50` loose UTF-16 guard) |
| `created_at` | DATETIME | Default CURRENT_TIMESTAMP |

Indexes on `seeker_name`, `giver_name`, `card_id`. Migration `migrateAddEntryNote` adds `note` via `ALTER TABLE ... ADD COLUMN` if missing (idempotent `pragma_table_info` check).
...
`Card` exposes `name`, `set`, `collector_number`, `colors`, `type_line`, `mana_value` (JSON `null` when unknown), `image_url`, `set_codes` (comma-separated sorted set codes of all printings of the card name; `""` = not yet backfilled) (JSON); `id` and sort keys are `json:"-"`. `Entry` exposes `id`, nested `card`, `seeker`, `giver` (`""` when NULL), `occasions` (array of occasion names inherited from the seeker, always present — `[]` when the seeker has none), `note` (printing note, `""` when none, always present), `created_at`. `Occasion` exposes `id`, `name`, `date_or_recurring`, `past` (computed server-side: true when non-recurring and date < today, server local date).
...
|---|---|---|---|
| `GET` | `/api/entries` | `200 {"entries":[…], "total":N}` (all entries, server-sorted; `note` always present in each entry; when `?user=<name>` is supplied, only entries where the seeker shares at least one occasion with the viewer are returned — entries with no occasions are always visible, viewers with no occasions see all) | — |
| `POST` | `/api/entries?user=<seeker>` | `201 {"created":N, "errors":[{"line",...}]}` (one card object per unit; duplicates allowed; per-card errors recorded, batch continues; card object may include optional `note` string — trimmed, truncated to 50 characters (runes)) | `400` bad JSON / empty user |
...
| `POST` | `/api/cards/setcodes` | `204` (bulk-updates `cards.set_codes` **by name only** from body `{"cards":[{name,set_codes}]}` — all card rows sharing the name get the value; rejects empty names) | `400` bad JSON / empty name |
...
- **Screen B** (gallery): sticky top bar with "I'm seeking..." button (opens the add-cards modal), "My occasions" button (opens the My-occasions modal), card-name filter, set-code filter (narrow search input sized to the placeholder via `size="9"`; prefix match — an entry is kept when the typed code is a prefix of the requested `set` or of any stored `set_codes` printing code, so `MH` matches `MH3`; falls back to the requested `set` before backfill completes; pre-filled from a `?set=<code>` URL param on page load and popstate, still editable afterwards — edits are not written back to the URL), color filter, card-type filter, occasion multiselect, seeker-name multiselect, giver-name multiselect, "Hide giver found" checkbox, sort dropdown (far right). Grid of all filtered tiles (no pagination; off-screen images stay deferred via `loading="lazy"`). On mobile (≤768px) the top bar collapses behind a single `☰ menu` toggle: only the toggle is visible by default, and tapping it adds `.expanded` to `.topbar` to reveal the other 10 controls stacked full-width; tapping again (or opening a control) collapses back and closes any open dropdowns via `closeAllPanels()`. The toggle is hidden on desktop (>768px).
- **Tile**: Scryfall image displayed via plain `<img src=card.image_url loading="lazy">`. The `image_url` stored in the DB is a direct `cards.scryfall.io` URL (no rate limit), so images load instantly with no queue or 429 risk. If `image_url` is empty (legacy card not yet backfilled), it falls back to the `/cards/named` redirect URL. `onerror` swaps to a "no image" placeholder. The `.card-tile` has a subtle `1px #2a3450` frame around image, note, and info. Directly below the image a `.note` pill renders the entry's `note` (e.g. "English, M15 frame") when non-empty — tightly wrapped (`width:fit-content`, `margin:6px auto`, `align-self:center`, `padding:4px 12px`, `border-radius:999px`, `overflow-wrap:anywhere`), bold `12px` white on accent `#e94560` — then an info box holds each line on its own row: `seeker: <name>`, `giver: <name>` (only rendered when a giver exists), then conditional action buttons (only rendered when non-empty). The `.card-tile` is a flex column and `.info` is `flex:1`, so within a grid row all info boxes stretch to the tallest tile's height. `.info` is itself a flex row: an `.info-text` column (seeker/giver/actions, `justify-content:center`) on the left and the trash button on the right, with `align-items:center` on `.info` so the button is vertically centered against the whole text block even when a giver row is present. When a giver exists the info box gets a thick bright-green border (`#22c55e`) and the giver line text turns bright-green bold so already-given cards are easy to spot.
...
- **Add-cards modal**: the "I'm seeking..." button in the top bar opens a centered modal overlay (backdrop click or Escape closes) containing a heading "I'm looking for the following cards...", a CubeCobra hint line, a textarea, an optional note input (code-point counter `0/50`, JS truncates to 50 characters, Enter submits, e.g. "English, M15 frame") that applies to every card in the batch, a subheading "I'm going to these occasions..." above an occasion picker (shared `renderOccasionPicker` with the My-occasions modal, pre-checked with the user's occasions) between the textarea and the "Add cards" button, plus progress + results. The "Add cards" button stays disabled until at least one occasion is checked and the textarea is non-empty. On submit, changed occasion selections are saved via `POST /api/occasions/mine` first, then entries post (card objects now include optional `note`), still no per-card occasion data — the server enriches entries at list time. A hint line below the heading tells CubeCobra users how to produce a paste-ready list: filter 'status:"not owned"', Export, Card Versions (.txt), copy-paste. Parsing: split on newlines; each line is `[qty] Name [(SET) [collector_number]]` — quantity, set code, and collector number are all optional; collector number requires a set code. Quantity prefix: `2 Name` / `2x Name`. Examples: `Faerie Guidemother`, `1 Faerie Guidemother (ELD)`, `Faerie Guidemother (ELD) 11`. Duplicates and quantity-prefix both expand to separate units. Metadata is resolved per unique `(name,set,collector_number)` from Scryfall via the batched `POST /cards/collection` endpoint (up to 75 identifiers per request, 500ms between batches); when a collector number is present the identifier uses `{set, collector_number}` (exact printing), otherwise `{name}` or `{name, set}`. A 270-card list is ~4 requests instead of 270. Unresolved lines are reported as errors and kept in the textarea; successful lines are cleared and the note input is cleared on full success. Results show "Added N cards." plus failed lines. Each resolved card's `colors`, `type_line`, `cmc` (stored as `mana_value`), and `image_uris.normal` are captured from Scryfall and sent with the batch note to the server.
- **Cards per row** (topbar, next to `sort`): a dropdown (`auto` / 2–12 per row) that shares one unified `cardsPerRow` state with pinch-to-zoom. `auto` keeps today's auto-fill layout (pinch adjusts `--card-min`, persisted in `localStorage.cardMin`); a numeric N fixes the column count via `repeat(N, minmax(0, 1fr))` (`.card-grid.fixed-cols` sets `--cols`), and pinch bumps N (in = more/smaller, out = fewer/larger, clamped 2–12), syncing the dropdown. Selection persists in `localStorage.cardsPerRow`. In `auto` mode the dropdown option label reflects the currently laid-out column count (e.g. `auto (4 per row)`), computed from the grid width and `--card-min` (`floor((width + gap) / (cardMin + gap))`) and refreshed on render, on resize, and whenever grid sizing changes; it falls back to plain `auto` when the grid isn't measured (e.g. on the name screen).
- **Pinch-to-zoom** (mobile): a two-finger pinch on the grid resizes cards instead of page-zooming — pinch out → larger cards (zoom in), pinch in → smaller cards (zoom out, more cards per row). In `auto` mode the card min width (`--card-min`) is clamped to `[80, 400]` px and persisted in `localStorage` (`cardMin`); in numeric mode pinch adjusts the cards-per-row count instead (see above). `touch-action: pan-y` on `.card-grid` lets the browser keep vertical scrolling while the app handles the pinch gesture; single-tap tile clicks are unaffected.
...
- **Set-codes backfill**: on `loadEntries`, for each distinct card name whose `set_codes` is empty, the frontend calls `GET https://api.scryfall.com/cards/search?q=!"<name>"&unique=prints` (paced by the rate-limited queue, following `has_more`/`next_page` up to 10 pages), dedupes + sorts the uppercase set codes, persists them via `POST /api/cards/setcodes` (server updates all card rows sharing the name), and updates local state + re-renders. Cards Scryfall can't resolve stay empty and are retried on later loads (same as metadata backfill).
...
- `popstate` re-syncs the screen from the URL so back/forward between `?user=` and `/` switches screens. Occasion selection syncs to the URL as repeated `?occasion=<name>` params (preserved alongside `?user=`); other filter changes leave the URL untouched — a `?set=<code>` param only ever pre-fills the set-code filter (on page load and popstate) and is not updated by filter edits.
...
```sh
CGO_ENABLED=1 go test ./server/tests/   # 69 tests, ~2s
```

Integration tests use a shared in-memory SQLite DB (`server.InitTestDB` + `ResetTestDB` via `TestMain` in `helpers_test.go`, single connection, `synchronous=OFF`/`journal_mode=MEMORY`) + `httptest` recorder. DB tests stay sequential on the shared DB; sort-key unit tests are `t.Parallel` (no DB). The 6 migration tests still use temp-file DBs via `server.InitDB` (they need close/reopen persistence). Production `server.InitDB` shares the same `schemaSQL` + migrations. Coverage includes: sort-key unit tests (including creature-wins-over-artifact/enchantment), batch add (entry/card counts, stored sort keys), duplicate lines, collector-number distinguishing printings, collector-number collapse without printing spec, collector-number in list response, image-URL backfill by collector number, list ordering across colors/types with mana-value then name tie-break, metadata backfill updating `mana_value` and re-sorting, `mana_value` migration converting a legacy NOT NULL DEFAULT 0 column to nullable with `0 → NULL`, the one-time `type_sort_key` recompute migration (creature-wins recomputation from stored `type_line`, idempotent via the `schema_meta` marker), occasions create/list/past-flag/duplicate/invalid-date/missing-user, seeker occasions set/replace-all/reject-empty/unknown-id/missing-user, the `hedwig` seed migration (links pre-existing seekers, idempotent), entries exposing occasion names (and `[]` when the seeker has none), note stored/exposed/trimmed/truncated (50-character cut), same card with different notes => 1 card row + N entries (cross-seeker and same-seeker variants), note persists across giver ops and is present even when empty, `entries.note` legacy migration via DROP COLUMN + re-InitDB, `cards.set_codes` legacy migration (DROP COLUMN + re-InitDB re-adds it, legacy rows default `""`), set-codes backfill updating all card rows sharing a name while leaving other cards untouched, self-offer, idempotent re-claim, 409-with-entry conflict, 404 after removal, clear-giver ownership, seeker/giver/non-party removal (204/403/404), the full-list pagination contract (200 entries returned in order), occasion isolation (disjoint occasions hidden when `?user=` supplied, shared/empty/no-occasion edge cases, and client default-filter by viewer's occasions).
...
- Server filters `GET /api/entries?user=` by occasion overlap (entries with disjoint occasions are omitted); anonymous fetches still return all entries, and large galleries still return the full filtered set (no pagination).
