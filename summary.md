...
`Card` exposes `name`, `set`, `collector_number`, `colors`, `type_line`, `mana_value` (JSON `null` when unknown), `image_url` (JSON); `id` and sort keys are `json:"-"`. `Entry` exposes `id`, nested `card`, `seeker`, `giver` (`""` when NULL), `occasions` (array of occasion names inherited from the seeker, always present — `[]` when the seeker has none), `note` (printing note, `""` when none, always present), `created_at`. `Occasion` exposes `id`, `name`, `date_or_recurring`, `past` (computed server-side: true when non-recurring and date < today, server local date).
